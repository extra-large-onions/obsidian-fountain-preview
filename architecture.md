...
**Single file.** Command **"Export current .fountain to PDF"** (`exportFilePdf()`, also on the `file-menu` as *Export to PDF*) runs the same renderer over one file only and writes `<stem>.pdf` beside the source, stripping any `.fountain` still carried in `basename` (as in `foo.fountain.txt`). No language filtering applies — the file you picked is the file you get. The command uses a `checkCallback` so it only appears when the active file is a `.fountain`.
